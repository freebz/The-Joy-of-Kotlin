...
fun assertTrue(condition: Boolean,
    ...
    Result<Boolean> = assertCondition(condition, { x -> x }, message)
...
fun <T> assertNotNull(t: T):
    Result<T> = assertNotNull(t, "Assertion error: object should not be null")

fun <T> assertNotNull(t: T, message: String):
    Result<T> = assertCondition(t, { x -> x != null }, message)

fun <T> assertPositive(value: Int,
                       message: String = "Assertion error: value $value must be positive"):
    Result<Int> = assertCondition(value, { x -> x > 0 }, message)

fun assertInRange(value: Int, min: Int, max: Int):
    Result<Int> = assertCondition(value, { x -> x in min..(max - 1) },
...
fun <A: Any> assertType(element: A, clazz: Class<*>):
    Result<A> = assertType(element, clazz,
        "Wrong type: ${element.javaClass}, expected: ${clazz.name}")

fun <A: Any> assertType(element: A, clazz: Class<*>,
                        message: String):
    Result<A> = assertCondition(element, { e-> e.javaClass == clazz }, message)
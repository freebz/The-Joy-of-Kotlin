...
sealed class List<out A> {
    ...
    companion object {
        ...
        fun <A> cons(a: @UnsafeVariance A, list: List<A>): List<A> = list.cons(a)
...
fun <A> zipWithPosition(list: List<A>): List<Pair<A, Int>> =
    zipWith(list, range(0, list.length)) { a -> { i: Int -> Pair(a, i) } }
